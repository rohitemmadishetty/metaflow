...
class TaskDataStore(object):
    ...
    def load_artifacts(self, names):
        ...
        """
        if not self._info:
            raise DataException("No info object available to retrieve artifacts")
        to_load = []
        sha_to_names = {}
        for name in names:
            ...
            if info:
                ...
                encode_type = "gzip+pickle-v2"
            if encode_type not in self._encodings:
                raise DataException("Python 3.4 or later is required to load %s" % name)
            else:
        ...
        def blob_iter():
            for name, value in contents.items():
                ...
                if isinstance(value, (RawIOBase, BufferedIOBase)) and value.readable():
                    ...
                    raise DataException(
                        "Metadata '%s' has an invalid type: %s" % (name, type(value))
                    )
